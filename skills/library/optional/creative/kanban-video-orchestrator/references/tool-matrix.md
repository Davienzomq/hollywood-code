...
Maps each role archetype to the Hollycode skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public hollycode
...
## Hollycode skills relevant to video production

### Visual / rendering skills (`hollycode/skills/creative/`)
...
### Design / pre-production skills (`hollycode/skills/creative/`)
...
### Audio / media skills (`hollycode/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`hollycode/skills/devops/`)
...
These are **not** Hollycode skills but external CLIs / APIs that profiles invoke.
...
## Built-in Hollycode tools for media review

These are native Hollycode tools — not invoked via terminal but through their own
...
key is present in `${HOLLYCODE_HOME:-~/.hollycode}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Hollycode profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HOLLYCODE_HOME:-~/.hollycode}/.env` → environment variable.
...
When a new Hollycode-public video skill ships:
