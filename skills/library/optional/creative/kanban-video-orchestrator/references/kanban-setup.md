...
the actual `setup.sh` that creates the project workspace, configures Hollycode
...
2. **Create profiles** — `hollycode profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hollycode/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hollycode kanban create` assigned to the director
...
```bash
hollycode profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hollycode/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hollycode/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Hollycode
...
Each profile gets a `SOUL.md` at `~/.hollycode/profiles/<name>/SOUL.md` that
...
```bash
hollycode kanban create "Direct production of <video title>" \
    --assignee director \
...
the Hollycode `.env` (`${HOLLYCODE_HOME:-$HOME/.hollycode}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hollycode_env="${HOLLYCODE_HOME:-$HOME/.hollycode}/.env"
    if grep -q "^${var}=" "$_hollycode_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_hollycode_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_hollycode_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hollycode ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hollycode OPENROUTER_API_KEY || exit 1
# ...
```
