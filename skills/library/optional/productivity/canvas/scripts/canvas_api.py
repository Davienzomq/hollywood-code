#!/usr/bin/env python3
"""Canvas LMS API CLI for Hollycode.
...
def _check_config():
    ...
    if missing:
        hollycode_env = os.path.join(
            os.environ.get("HOLLYCODE_HOME", os.path.expanduser("~/.hollycode")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {hollycode_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Hollycode"
    )
