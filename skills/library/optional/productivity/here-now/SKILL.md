...
- Skill helper paths:
  - `${HOLLYCODE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HOLLYCODE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HOLLYCODE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client hollycode
```
...
```bash
PUBLISH="${HOLLYCODE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client hollycode
```
...
```bash
DRIVE="${HOLLYCODE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `hollycode`)    |
