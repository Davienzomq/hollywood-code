...
- smoke-test a server with the FastMCP CLI before wiring it into Hollycode or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Hollycode. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hollycode/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hollycode/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Hollycode integration, either:

- configure the server in `~/.hollycode/config.yaml` using the `native-mcp` skill, or
...
### Hollycode cannot see the deployed server

The server-building part may be correct while the Hollycode config is not. Load the `native-mcp` skill and configure the server in `~/.hollycode/config.yaml`, then restart Hollycode.
