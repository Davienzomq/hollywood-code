...
version: 0.1.0
author: Matt Maximo (MattMaximo), Hollycode
license: MIT
platforms: [linux, macos, windows]
metadata:
  hollycode:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, hollycode]
---

# Grok Build CLI — Hollycode Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Hollycode terminal. Grok
...
1. Prepare stable input files first with Hollycode tools (`read_file`,
...
2. **Don't conflate Hollycode' xAI auth with the `grok` CLI's auth.** Hollycode'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Hollycode' xAI auth carries
   over.
