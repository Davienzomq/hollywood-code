...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Hollycode
license: MIT
platforms: [linux, macos]
metadata:
  hollycode:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, hollycode]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Hollycode.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Hollycode-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hollycode` (Hollycode subagents via `delegate_task`).
