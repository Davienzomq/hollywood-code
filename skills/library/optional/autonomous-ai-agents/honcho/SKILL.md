---
name: honcho
description: Configure and use Honcho memory with Hollycode -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Hollycode
license: MIT
...
# Honcho Memory for Hollycode

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Hollycode profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hollycode#running-honcho-locally-with-hollycode

### Verify

```bash
hollycode honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Hollycode profile's AI peer
...
Honcho models conversations as interactions between **peers**. Hollycode creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Hollycode instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Hollycode run |
| `global` | Single session across all directories |

Manual override: `hollycode honcho map my-project-name`
...
Each Hollycode profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
hollycode profile create coder --clone
# creates host block hollycode.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hollycode.coder` host block in `honcho.json`
...
```bash
hollycode honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Hollycode when Honcho memory is active.
...
Config file: `$HOLLYCODE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hollycode honcho setup`. Ensure `memory.provider: honcho` is in `~/.hollycode/config.yaml`.

### Memory not persisting across sessions
Check `hollycode honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hollycode profile create <name> --clone`. For existing profiles: `hollycode honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hollycode honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hollycode honcho status` | Show resolved config, connection test, peer info for active profile |
| `hollycode honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hollycode honcho disable` | Disable Honcho for the active profile |
| `hollycode honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hollycode honcho peers` | Show peer identities across all profiles |
| `hollycode honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hollycode honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hollycode honcho sessions` | List known directory-to-session-name mappings |
| `hollycode honcho map <name>` | Map current working directory to a Honcho session name |
| `hollycode honcho identity` | Seed AI peer identity or show both peer representations |
| `hollycode honcho sync` | Create host blocks for all Hollycode profiles that don't have one yet |
| `hollycode honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hollycode + Honcho |
| `hollycode memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hollycode memory status` | Show active memory provider and config |
| `hollycode memory off` | Disable external memory provider |
