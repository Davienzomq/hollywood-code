...
version: 0.1.0
author: Bihruze (Asahi0x), Hollycode
license: MIT
...
import upstream classes into Hollycode itself.
...
```bash
mkdir -p ~/.hollycode/cache/darwinian-evolver && cd ~/.hollycode/cache/darwinian-evolver
...
```bash
cd ~/.hollycode/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hollycode/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hollycode/skills/research/darwinian-evolver
DE_DIR=~/.hollycode/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hollycode core.
   Custom driver scripts under `~/.hollycode/skills/...` are user-side and fine.
