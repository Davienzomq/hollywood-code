...
description: Migrate a user's OpenClaw customization footprint into Hollycode. Imports Hollycode-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Hollycode (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hollycode:
    tags: [Migration, OpenClaw, Hollycode, Memory, Persona, Import]
    related_skills: [hollycode]
---

# OpenClaw -> Hollycode Migration

Use this skill when a user wants to move their OpenClaw setup into Hollycode with minimal manual cleanup.
...
```bash
hollycode claw migrate              # Full interactive migration
hollycode claw migrate --dry-run    # Preview what would be migrated
hollycode claw migrate --preset user-data   # Migrate without secrets
hollycode claw migrate --overwrite  # Overwrite existing conflicts
hollycode claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hollycode setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hollycode.py` to:

- import `SOUL.md` into the Hollycode home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Hollycode memory entries
- merge OpenClaw command approval patterns into Hollycode `command_allowlist`
- migrate Hollycode-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Hollycode working-directory configuration
- copy OpenClaw skills into `~/.hollycode/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hollycode workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hollycode/tts/`
- archive non-secret docs that do not have a direct Hollycode destination
...
- `scripts/openclaw_to_hollycode.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py`

Do not guess a shorter path like `~/.hollycode/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hollycode/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Hollycode-compatible secrets, currently:
...
Hollycode CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Hollycode. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Hollycode workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Hollycode skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hollycode/skills/migration/openclaw-migration/scripts/openclaw_to_hollycode.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Hollycode unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Hollycode targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Hollycode destination. Unsupported auth blobs must still be reported as skipped.
...
- Hollycode persona state imported
- Hollycode memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.hollycode/skills/openclaw-imports/`
