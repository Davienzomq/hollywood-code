...
version: 0.1.0
author: Teknium (teknium1), Hollycode
license: MIT
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.hollycode/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.hollycode/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:HollycodeCLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "hollycode code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.hollycode/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.hollycode/wikis/`. Only write into the repo when the user explicitly requests it.
