...
version: 1.0.0
author: arceus77-7, enhanced by Hollycode
license: MIT
...
- `tmux` available for stable authenticated sessions during Hollycode terminal calls (desktop app flow only)
...
### Service Account (recommended for Hollycode)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HOLLYCODE_HOME:-~/.hollycode}/.env` (the skill will prompt for this on first load).
...
## Hollycode Execution Pattern (desktop app flow)

Hollycode terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hollycode-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hollycode-op.sock"
