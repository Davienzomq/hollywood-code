...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HOLLYCODE_HOME", os.path.expanduser("~/.hollycode")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HOLLYCODE_HOME", Path.home() / ".hollycode")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Hollycode config paths
# ═══════════════════════════════════════════════════════════════════

HOLLYCODE_HOME = Path(os.getenv("HOLLYCODE_HOME", Path.home() / ".hollycode"))
CONFIG_PATH = HOLLYCODE_HOME / "config.yaml"
PREFILL_PATH = HOLLYCODE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Hollycode models — already uncensored, just needs clean prompt
    "hollycode": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hollycode" in model_lower or "nous" in model_lower:
        return "hollycode"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Hollycode config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hollycode/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Hollycode for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Hollycode for changes to take effect.")
