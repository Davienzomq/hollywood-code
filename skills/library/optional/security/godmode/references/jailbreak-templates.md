...
## 5. GODMODE FAST — Hollycode 4 405B (Zero Refusal)

**Model:** `nousresearch/hollycode-4-405b`
**Strategy:** Hollycode models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Hollycode

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.hollycode/config.yaml`:
