...
Usage (via cron with --no-agent):
    ...
    hollycode cron create hollycode-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HOLLYCODE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hollycode-issues --repo NousResearch/hollycode --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Hollycode .env file
(``${HOLLYCODE_HOME:-~/.hollycode}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Hollycode-Watcher/1.0",
    }
