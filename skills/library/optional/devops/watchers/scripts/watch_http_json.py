...
Usage (via cron with --no-agent):
    ...
    hollycode cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$HOLLYCODE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Hollycode-Watcher/1.0"})
