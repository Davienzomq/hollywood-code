---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. hollycode@agentmail.to).
version: 1.0.0
...
Add to `~/.hollycode/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `hollycode`)
   - The agent gets address: `hollycode@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "hollycode-outreach")
...
```
hollycode --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
