...
## 5. Hollycode Configuration

### Config location

`$HOLLYCODE_HOME/config.yaml` (defaults to `~/.hollycode/config.yaml` when `HOLLYCODE_HOME` is unset)
...
Restart the Hollycode session for changes to take effect. The MCP connection is
