...
HOLLYCODE_HOME_DIR="${HOLLYCODE_HOME:-$HOME/.hollycode}"
HOLLYCODE_CFG="${HOLLYCODE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Hollycode config has twozero_td MCP entry ──
if [[ ! -f "$HOLLYCODE_CFG" ]]; then
    echo -e " ${FAIL} Hollycode config not found at ${HOLLYCODE_CFG}"
    manual_steps+=("Create ${HOLLYCODE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HOLLYCODE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Hollycode config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Hollycode config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$HOLLYCODE_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HOLLYCODE_CFG} manually"); }
    manual_steps+=("Restart Hollycode session to pick up config change")
fi
