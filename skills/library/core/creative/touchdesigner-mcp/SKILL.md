...
metadata:
  hollycode:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, hollycode-video]
...
```
Hollycode -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${HOLLYCODE_HOME:-$HOME/.hollycode}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Hollycode config (if missing)
...
3. **Restart Hollycode session** to pick up the new MCP server
