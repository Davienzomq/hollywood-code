#!/usr/bin/env bash
# GitHub environment detection helper for Hollycode skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _hollycode_env="${HOLLYCODE_HOME:-$HOME/.hollycode}/.env"; [ -f "$_hollycode_env" ] && grep -q "^GITHUB_TOKEN=" "$_hollycode_env" 2>/dev/null; then
