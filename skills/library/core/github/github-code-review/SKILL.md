...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hollycode_env="${HOLLYCODE_HOME:-$HOME/.hollycode}/.env"; [ -f "$_hollycode_env" ] && grep -q "^GITHUB_TOKEN=" "$_hollycode_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hollycode_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Hollycode\",
    \"comments\": [
...
```bash
source "${HOLLYCODE_HOME:-$HOME/.hollycode}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Hollycode. Code looks clean — good test coverage, no security concerns."
