...
metadata:
  hollycode:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, debugging-hollycode-tui-commands]
---
...
- A long-running process (hollycode gateway, tui_gateway) misbehaves and you can't restart it
...
The hollycode test runner and pytest both support this:
...
For long-lived processes: Hollycode gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/hollycode/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Hollycode you have two practical options:
...
```json
{
  "name": "Attach to Hollycode",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/hollycode" }
  ]
}
```
...
## Debugging Hollycode-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `hollycode` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `hollycode --tui`)
...
debugpy.wait_for_client()
```
Start `hollycode --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Hollycode subagents, debug one process at a time.
