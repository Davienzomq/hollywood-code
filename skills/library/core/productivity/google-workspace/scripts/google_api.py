#!/usr/bin/env python3
"""Google Workspace API CLI for Hollycode.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Hollycode-facing JSON contract and falls back to the Python client
...
from _hollycode_home import get_hollycode_home
...
def _gws_binary() -> str | None:
    override = os.getenv("HOLLYCODE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Hollycode")
