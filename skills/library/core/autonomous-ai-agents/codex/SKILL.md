...
metadata:
  hollycode:
    ...
    related_skills: [claude-code, hollycode]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Hollycode terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Hollycode itself, `model.provider: openai-codex` uses Hollycode-managed Codex
OAuth from `~/.hollycode/auth.json` after `hollycode auth add openai-codex`. For the
...
## Hollycode Gateway Caveat

When invoking the Codex CLI from a Hollycode gateway/service context (for example,
