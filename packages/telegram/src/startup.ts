...
// The launcher (bin/hollycode-remote.ts) is what runs on startup: it loads the
// saved config, spawns the long-lived bridge, then exits. Keeping the boot
// entrypoint identical to a normal manual launch means /move, /model and the
// rest of the persisted config are honoured automatically.
const SELF = fileURLToPath(import.meta.url)
const LAUNCHER = path.resolve(path.dirname(SELF), "..", "bin", "hollycode-remote.ts")
...
function installWindows(directory: string) {
  ...
  const script = [
    ...
    `Register-ScheduledTask -TaskName '${TASK_NAME}' -Action $a -Trigger $t -Principal $p -Settings $s -Description 'Start Hollycode Remote (Telegram bot) at logon.' -Force | Out-Null`,
  ].join("; ")
...
function installMac(directory: string) {
  ...
  // launchd keeps the bridge alive, so run it in the foreground (no self-detach).
...
function installLinux(directory: string) {
  ...
  // systemd supervises the process, so run the bridge in the foreground.
  const unit = `[Unit]
Description=Hollycode Remote (Telegram bot)
...
/** Register the bot to start automatically on boot/logon for the current OS. */
