...
export async function createEngine(config: GatewayConfig): Promise<{
  ...
  try {
    ...
    if (defaultModel) log("engine", `Model auto-detected: ${defaultModel.providerID}/${defaultModel.modelID}`)
  } catch {
  ...
  const switchDir = async (dir: string, responder: Responder) => {
    ...
    await responder.sendText(`✅ Switched to ${dir}`).catch(() => {})
  }
  ...
  const handleMessage = async (channelId: string, msg: { conversationId: string; userId: string; text: string }, responder: Responder) => {
    ...
    const promptBody: any = { parts: [{ type: "text", text: promptText }] }
  ...
  const handleCommand = async (
    ...
    switch (command) {
      ...
      case "model": {
        ...
        // flatten to "providerID/modelID" labels
        const modelOptions: string[] = []
        for (const p of providers) {
          for (const k of Object.keys(p.models || {})) {
            modelOptions.push(`${p.id}/${k}`)
          }
        }
        if (!modelOptions.length) { await responder.sendText(`Current: ${cur}\nNo models available.`); break }
        const chosen = await responder.askQuestion({
          question: `🤖 Current: ${cur}\n\nSelect a model:`,
          options: modelOptions,
        })
        // chosen is "providerID/modelID"
        const chosenParts = chosen.split("/")
        if (chosenParts.length >= 2) {
          defaultModel = { providerID: chosenParts[0]!, modelID: chosenParts.slice(1).join("/") }
          config.model = chosen
          saveGatewayConfig(config)
      ...
      case "move": {
        ...
        // no arg: pick from desktop folders
        const desk = path.join(os.homedir(), "OneDrive", "OneDrive - Bedroom Elegance", "Desktop")
        let folders: string[] = []
        try {
          folders = fs.readdirSync(desk).filter((f) => fs.statSync(path.join(desk, f)).isDirectory())
        } catch { /* best-effort */ }
        if (!folders.includes(DIRECTORY)) folders.unshift(DIRECTORY)
        const options = folders.slice(0, 20).map((f) =>
          `${f === DIRECTORY ? "📍 " : ""}${path.basename(f)} [${f}]`,
        )
        if (!options.length) { await responder.sendText("No folders found. Use /move <path>"); break }
        const chosen = await responder.askQuestion({ question: "📂 Select a directory:", options })
        // extract path from label "[...]"
        const m = chosen.match(/\[(.+)\]$/)
        if (m) await switchDir(m[1]!, responder)
        break
      }
